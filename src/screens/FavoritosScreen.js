...
export default function FavoritosScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const unsubscribeFav = onSnapshot(
      query(collection(db, 'favorites'), where('uid', '==', usuario.uid)),
      (snapshot) => {
        favIds = snapshot.docs.map((doc) => doc.data().produtoId);
        atualizar();
      },
    );
    ...
    function atualizar() {
      const filtrados = produtosRef.filter((item) => favIds.includes(item.id));
